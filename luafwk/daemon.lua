	...
	local function supertask()
		while true do
			...
			if M.tokill[cell] then 
				log ('daemon', 'INFO', "Daemon thread %s killed.", name)
				M.tokill[cell] = nil
				return
			elseif not status then log ('daemon', 'WARNING', 
				"Relaunch daemon thread %s which failed with error %q", name, r)
			else log ('daemon', 'INFO', 
				"Relaunch daemon thread %s which terminated without error", name)
			end
			if delay then
				log ('daemon', 'INFO', "Wait %i seconds before relaunching", delay) 
				sched.wait(delay) 
			end
		end
	end
