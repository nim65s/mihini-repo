...
#define MT_NAME "bysant.core.dctx"
#define CKSTACK(L, x) luaL_checkstack( L, x, "Bysant deserializer")

static void bysant2lua( lua_State *L, const bsd_data_t *x) {
...
#		undef CASE
		...
		lua_error( L);
		break; /* dead code, to avoid IDE warnings */

	case BSD_CLASSDEF: return; // nothing to do

	default: luaL_error( L, "Unhandled type: %d", x->type);
	}
...
int luaopen_bysant_core_deserialize( lua_State *L) {
	luaL_findtable( L, LUA_GLOBALSINDEX, "bysant.core", 14); // bysant.core

	/* set bysant.niltoken if absent */
	lua_getglobal( L, "bysant");        // bysant.core, bysant
	lua_getfield( L, -1, "niltoken");   // bysant.core, bysant, ?niltoken
	if( lua_isnil(L, -1)) {
		lua_pop( L, 1);                   // bysant.core, bysant
		lua_getglobal( L, "require");     // bysant.core, bysant, require
		lua_pushstring( L, "niltoken");   // bysant.core, bysant, require, "niltoken"
		lua_call( L, 1, 1);               // bysant.core, bysant, niltoken
		lua_setfield( L, -2, "niltoken"); // bysant.core, bysant[niltoken=niltoken]
	}
	else lua_pop( L, 1); // bysant.core, bysant
	lua_pop( L, 1);       // bysant.core

	luaL_newmetatable( L, MT_NAME);        // bysant.core, mt
	lua_pushcfunction( L, api_collect); lua_setfield( L, -2, "__gc");
	lua_createtable( L, 0, 3);             // bysant.core, mt, __index


	lua_pushcfunction( L, api_deserialize);// bysant.core, mt, __index, deserialize
	lua_pushvalue( L, -1);                 // bysant.core, mt, __index, deserialize, deserialize
	lua_setfield( L, -3, "deserialize");   // bysant.core, mt, __index[deserialize], deserialize
	lua_setfield( L, -3, "__call");        // bysant.core, mt[__call=deserialize], __index
	...
	lua_setfield( L, -2, "__index");           // bysant.core, mt[__index]
	lua_pushstring( L, "bysant.deserializer"); // bysant.core, mt, "bysant.deserializer"
	lua_setfield( L, -2, "__type");            // bysant.core, mt[__type="bysant.deserializer"]
	lua_pop( L, 1);                            // bysant.core

	lua_pushcfunction( L, api_init);       // bysant.core, deserializer
	lua_pushvalue( L, -1);                 // bysant.core, deserializer, deserializer
	lua_setfield( L, -3, "deserializer");  // bysant.core[deserializer], deserializer
