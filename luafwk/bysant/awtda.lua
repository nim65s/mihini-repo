...
--- Creates a new AWTDA3 serializer.
-- It differs from a plain Bysant serializer in that a series of standard
-- AWTDA classes are predefined before it's used.
--
...
-- @usage
--
-- awtda = require 'bysant.awtda'
-- serializer1, acc = awtda.serializer{ }
-- serializer1 :number(1234)
-- printf("the serialized representation of 1234 is %q", table.concat(acc)) 
-- serializer2 = awtda.serializer(function(x) printf("Received %q", x"); return #x end)
...
--- Creates a new AWTDA deserializer object. AWTDA standard classes are predefined
--  in the deserializer.
--
...
--  content into an AWTDA envelope.
--
...
--     local s1, t1 = awtda.serializer{ }
--     assert(s1(payload))
--     local serialized_payload = table.concat(t1)
--     local s2, t2 = awtda.serializer{ }
--     assert(s2{
...
function M.envelope(header, footer)
    ...
    local function stream_header()
        log('BYSANT-AWTDA', 'DEBUG', "Streaming header")
    ...
    local function stream_footer()
        log('BYSANT-AWTDA', 'DEBUG', "Streaming footer")
    ...
    local function filter (chunk)
        ...
        if chunk ~= nil and chunk ~= '' then
            log('BYSANT-AWTDA', 'DEBUG', "Streaming %d bytes of payload", #chunk)
        ...
        if buf_size > SINGLE_MAX_SIZE or several_chunks and buf_size >= CHUNK_SIZE then
            ...
            if not several_chunks then -- switch to multi-chunk mode
                log('BYSANT-AWTDA', 'DEBUG', "Switch to multi-chunk encoding")
                serializer :chunked()
                several_chunks = true
            end
            ...
            log('BYSANT-AWTDA', 'DEBUG', "Flushing %d bytes of payload chunk", #left)
...
global 'bysant'; if type(bysant) == 'table' then bysant.awtda = M end
