...
#define MT_NAME "bysant.core.sctx"
...
static int api_call( lua_State *L) {
	...
	luaL_findtable( L, LUA_GLOBALSINDEX, "bysant.core", 0); // ctx, x, bysant.core
	lua_getfield( L, -1, "value"); // ctx, x, bysant.core, value
	lua_pushvalue( L, -4);         // ctx, x, bysant.core, value, ctx
	lua_pushvalue( L, -4);         // ctx, x, bysant.core, value, ctx, x
	lua_call( L, 2, 2);            // ctx, x, bysant.core, statue, ?errmsg
	if(lua_isnil( L, -2)) {        // ctx, x, bysant.core, nil, errmsg
		return 2;
	} else {                       // ctx, x, bysant.core, status_ok, nil
		lua_pop( L, 1);             // ctx, x, bysant.core, status_ok
		return 1;
	}
}

/* Open the module; to be stored in package.preload['bysant.core']*/
int luaopen_bysant_core_serialize( lua_State *L) {
...
#undef REG
  ...
  /* Set bysant.core as the methods table for bysant contexts. */
  luaL_newmetatable( L, MT_NAME);  // bysant.core, mt
  // set cleanup method
  lua_pushcfunction( L, api_collect); lua_setfield( L, -2, "__gc");
  lua_pushvalue( L, -2);           // bysant.core, mt, bysant.core
  lua_setfield( L, -2, "__index"); // bysant.core, mt[__index=bysant.core]
  lua_pushcfunction( L, api_call); // bysant.core, mt, __call
  lua_setfield( L, -2, "__call");  // bysant.core, mt[__call]
  lua_pushstring( L, "bysant.serializer"); // bysant.core, mt, "bysant.serializer"
  lua_setfield( L, -2, "__type");    // bysant.core, mt[__type="bysant.serializer"]

  lua_pop( L, 1);                  // bysant.core

  return 1; // bysant.core
}
