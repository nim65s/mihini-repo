...
function MT :get(hpath)
    ...
    local c = self.nlnodes[hpath]
    if c then
        local children_set = { }
        for k, _ in pairs(c) do children_set[c] = true end
        return nil, children_set
    else return nil, nil end
end

--- Store this set of path/value pairs, notify a change if appropriate.
--  The paths are used as keys in `self.values`. If a path, o one of
--  its prefixes, is registered for monitoring, then a notifiactionis
...
--- Create a new ram handler with the specified name.
