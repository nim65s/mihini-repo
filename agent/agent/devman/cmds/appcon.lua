...
return function(asset, args, fullpath, ticket)
    ...
    for subcommand, subargs in utable.sortedpairs(args) do
        ...
        local appname = subargs[1] or subargs.appname or error("bad app name")
        if subcommand=='start' then
            assert(appcon.start(appname))
        elseif subcommand=='stop' then
            assert(appcon.stop(appname))
        elseif subcommand=='autostart' then
            local autostart = subargs[2] -- don't use and/or idiom, autostart might be 'false'
            if autostart==nil then autostart = subargs.autostart end
