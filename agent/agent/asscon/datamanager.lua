...
function handle.TableNew(asset, x)
    ...
    if t then
        ...
        if t.src_table then return 11, "cannot create or retrieve "..table_id.." because it is a destination table." end
        if x.purge then
...
function handle.PData(asset, x)
    ...
    local function put_in_records(path, record)
        ...
        if not columns then return 11, with_subrecords end -- support error
        if not with_subrecords then
            ...
            for k, v in pairs(record) do
                if type(v)=='table' then
                    ...
                    if not r then return 1, err end
    ...
    if type(record)=='table' then
        ...
        if x.path == '' then return 11, "no path and no key" end
...
function handle.ConsoNew(asset, x)
    ...
    if not CP then return 11, "no such consolidation policy" end
