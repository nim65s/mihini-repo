...
-- testsuites format:
--  |-running          => if true, the testsuite shall be resumed
--  |-suites
testsuites = persist.table.new('testsuites', 250)

local testfailuretoken = {}

-- continue tests after a reboot. Do nothing if the tests have not been started
function resumeTests()
  if testsuites.running then
    -- continue tests
  else
    -- nothing to do: tests not started or already finished
  end
end
...
function assert_true(got, msg)
   wraptest(got==true, msg, "Expected true")
end
...
function assert_clone_tables(exp, got, msg)
    ...
    wraptest(nil == next(d), msg, fmt("Expected clone tables, but keys { %s } are different", table.concat(d, ', ')))
end
