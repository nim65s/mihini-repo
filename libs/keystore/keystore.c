...
static int set_obfuscated_bin_keys( int key_index, int n_keys, unsigned const char *obfuscated_bin_keys);
 ...
 * @param idx_K key index of the main password key
 ...
 *
 * @param key_index the index of the key whose (des)obfuscation is requested.
 *   different key indexes should have different obfuscated representations,
 *   to avoid amking it obvious when the same key is used at several places.
 ...
 * @return CRYPT_OK or CRYPT_ERROR.
 */
static int get_ecb_obfuscator( symmetric_ECB *ecb_ctx, unsigned const char *obfuscation_bin_key) {
 ...
 *
 * @param key_index index of the key to retrieve.
 ...
 *
 * @param key_index index of the first key to write.
 * @param n_keys number of keys to write.
 * @param plain_bin_keys keys to write, concatenated in order.
 * @return CRYPT_OK or CRYPT_ERROR.
 */
int set_plain_bin_keys(int key_index, int n_keys, unsigned const char *plain_bin_keys) {
    ...
    if( set_obfuscated_bin_keys( key_index, n_keys, obfuscated_bin_keys)) goto cleanup;
 ...
 *
 * @param first_index index of the first key to write.
 * @param n_keys number of keys to write.
...
static int set_obfuscated_bin_keys( int first_index, int n_keys, unsigned const char *obfuscated_bin_keys) {
    ...
    if ( ! file) { /* empty file, size=0 */
        ...
        if( fseek( file, 0, SEEK_END)) return CRYPT_ERROR;
        sz = ftell( file);
        content = malloc( sz);
        if( ! content) { fclose( file); return CRYPT_ERROR; }

        /* Read the whole file into `content`. */
        if( ! fseek( file, 0, SEEK_SET) ||
                fread( content, 1, sz, file) != sz) {
            fclose( file);
            memset( content, 0, sz);
            free( content);
            return CRYPT_ERROR;
    ...
    if( sz < needed_sz) {
         ...
         * in case of failure. */
        unsigned char *new_content = malloc( needed_sz);
        if( ! new_content) {
            memset( content, 0, sz);
            free( content);
            return CRYPT_ERROR;
    ...
    unsigned char *obfuscated_hex_key = content + 33*(first_index-1);
    int i; for( i = 0; i < 16 * n_keys; i++) {
        const char *fmt = (i+1)%16 ? "%02x" : "%02x\n";
        sprintf( (char *) obfuscated_hex_key + 2*i, fmt, obfuscated_bin_keys[i]);
    }
    ...
    if( file) {
        success = (fwrite( content, sz, 1, file) == sz);
        fclose( file);
    } else {
    ...
    memset( content, 0, sz);
    free( content);
    return success ? CRYPT_OK : CRYPT_ERROR;
}
